...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const unlistenDownload = listen<DownloadData>('download-progress', (event) => {
  ...
  const checkStatus = async () => {
    try {
      const status: any = await invoke('get_game_status');
  ...
  const handleSelectDir = async () => {
    const newDir: string | null = await invoke('select_directory');
    if (newDir) {
      setGameDir(newDir);
      await checkStatus();
    }
  ...
  const handleToggleAddon = async () => {
    ...
    try {
      await invoke('toggle_addon', { enabled: nextState });
    } catch (error) {
  ...
  const openFolder = () => invoke('open_game_folder');
      ...
      <main>
        ...
        <div className="options-grid">
            ...
            <div 
              ...
              onClick={() => invoke('open_external', { url: 'https://discord.gg/kv6hCjvMbp' })}
            >
